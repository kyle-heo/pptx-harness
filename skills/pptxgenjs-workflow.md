...
## Step 3.5: 콘텐츠 충실도 설계 (빈 슬라이드 금지)

신규 슬라이드는 **절대 비어 보이면 안 된다.** 스펙이 한 줄짜리 키워드만 줬더라도 그대로 한 줄만 박지 말고, 슬라이드의 역할에 맞게 합리적으로 살을 붙여 채운다.

### 슬라이드별 최소 콘텐츠 기준
| 역할 | 최소 구성 요소 |
|------|---------------|
| cover | 타이틀 + 부제(또는 한 줄 설명) + 발표자/날짜/조직 중 1 |
| section divider | 섹션 번호 + 섹션명 + 한 줄 인트로 |
| content (two-col) | 좌측 본문 3~5줄 (제목 + 설명 2~4줄) + 우측 시각 요소(카드/아이콘/도형) |
| grid | 카드 4~6개, 각 카드 = 아이콘/숫자 + 한 줄 제목 + 한 줄 설명 |
| stat-callout | 대형 숫자 + 단위/라벨 + 1~2줄 컨텍스트 + 보조 지표 1개 |
| timeline | 단계 3~5개, 각 단계 = 라벨 + 한 줄 설명 |
| closing | 메인 메시지 + CTA(또는 연락처/Q&A) + 짧은 마무리 문구 |

### 콘텐츠 채우기 원칙
1. **합리적 추론으로 살 붙이기** — 스펙·브랜드 가이드·참고 PPT 톤에서 유도되는 자연스러운 보조 문구를 직접 작성한다. 단순 반복·동의어 나열·"내용 채우기용 더미"는 금지.
2. **추론한 내용은 명시** — 스펙에 없는데 새로 써넣은 텍스트는 Step 3 플랜에 `(추론)` 또는 `(보강)` 표시. 사용자 검토가 가능해야 한다.
3. **숫자·고유명사 환각 금지** — 통계, 가격, 인용, 회사명/제품명은 스펙·context에 명시된 것만 사용. 없으면 대체 표현 (`예: ~의 경우`, `통상 ~수준` 등) 또는 시각 요소로 대체.
4. **시각 요소로 빈 공간 메우기** — 텍스트가 부족한 슬라이드는 아이콘, 카드, 일러스트, 도형, 차트 placeholder 등으로 시각적 균형을 맞춘다. 빈 흰 공간 > 30% 면 재설계.
5. **placeholder 텍스트 잔류 금지** — `lorem ipsum`, `xxx`, `TODO`, `여기에 내용`, `[insert ...]` 등은 P-010(Step 5)에서 자동 검출되어 실패 처리.

### 결과물
Step 3 슬라이드 플랜에 각 슬라이드의 **실제 들어갈 텍스트 초안**을 한 번 미리 적어둔다. Step 4에서 코드를 작성할 때는 이 초안을 그대로 옮긴다 (즉흥 작성 금지).
